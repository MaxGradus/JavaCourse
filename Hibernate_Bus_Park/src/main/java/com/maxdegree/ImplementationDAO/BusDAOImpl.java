...
//            Query query = session.createSQLQuery("SELECT * FROM busses WHERE drivers.driver_id LIKE : driverID");
    ...
    public Collection getBussesByRoute(Route route) throws SQLException {
        ...
        try {
            ...
            Query query = session.createSQLQuery("SELECT * FROM busses WHERE routes.route_id LIKE : routeId");
            busses = (List<Bus>)query.setLong("routeId", route_id);
            session.getTransaction().commit();
        } finally {
            if (session != null && session.isOpen()) {
                session.close();
